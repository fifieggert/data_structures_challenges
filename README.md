...
Plataforma utilizada: LeetCode
Estrutura de dados principal usada: Conjunto (Set) e Lista Encadeada (Linked List).Justificativa: O problema exige identificar grupos de nós conectados que existam dentro de um subconjunto (nums). O Set é a estrutura mais adequada aqui pois permite verificar se um valor do nó pertence ao subconjunto em tempo constante $O(1)$. Combinado com o percurso sequencial da Lista Encadeada, conseguimos detectar o início de um novo componente sempre que um valor presente no Set é encontrado após um valor ausente (ou no início da lista).
